...
import { resolveTelegramCallbackAction } from "./callback-actions.js";
...
export const registerTelegramHandlers = ({
  ...
  bot.on("callback_query", async (ctx) => {
    ...
    try {
      ...
      const action = await resolveTelegramCallbackAction({
        cfg,
        accountId,
        data,
        chatId,
      ...
      if (action.kind === "edit") {
        const keyboard = buildInlineKeyboard(action.buttons);
        try {
